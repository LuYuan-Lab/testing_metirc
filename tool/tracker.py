...
class VideoTracker:
    ...
    def _update_matched_tracks(self, matched_tracks: List, detections: List):
        ...
        for track_id, det_idx in matched_tracks:
            ...
            self.tracks[track_id]["history"].append({"frame": self.frame_count, "box": det[:4], "conf": det[4]})
    ...
    def _init_new_tracks(self, unmatched_dets: List, detections: List):
        ...
        for det_idx in unmatched_dets:
            ...
            self.tracks[new_track_id] = {
                ...
                "lost_frames": 0,
                "history": [{"frame": self.frame_count, "box": det[:4], "conf": det[4]}],
            }
...
# 便捷函数
def create_tracker(
    enable_tracking: bool = True, tracker_type: str = "bytetrack", track_buffer: int = 30, **kwargs
) -> VideoTracker:
    """
    创建跟踪器的便捷函数
    ...
    config = TrackingConfig(
        enable_tracking=enable_tracking, tracker_type=tracker_type, track_buffer=track_buffer, **kwargs
    )
...
class UltralyticsTracker:
    ...
    def track(self, frame: np.ndarray, target_class: str = "person") -> Dict[int, Tuple[float, float, float, float]]:
        ...
        # 使用内置跟踪功能
        results = self.model.track(frame, tracker=self.tracker_config, verbose=False, persist=True)  # 保持跟踪状态
...
# 导出的主要接口
__all__ = ["TrackingConfig", "VideoTracker", "UltralyticsTracker"]
