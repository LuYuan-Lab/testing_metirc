...
def track_and_visualize(input_video, output_dir="visualization/output_visualization/tracking", 
                       confidence_threshold=0.3, max_frames=None, trail_length=30):
    """跟踪视频并保存可视化结果"""
    ...
    try:
        ...
        print(f"📈 处理了 {frame_count} 帧，检测 {total_detections} 次，跟踪 {total_tracks} 次，{len(track_ids)} 个唯一ID")
...
def visualize_tracking(
    ...
    for track_id, track_info in tracks.items():
        if isinstance(track_info, dict):
            ...
            state = track_info.get('state', 'active')
        else:
            # 兼容旧格式 (x1,y1,x2,y2)
            bbox = track_info
            confidence = 1.0
    ...
    # 绘制帧信息
    if frame_info:
        info_text = f"{frame_info} | Det: {len(tracks)} | Tracks: {len([t for t in tracks.values() if isinstance(t, dict) and t.get('state') == 'active'])} | IDs: {len(tracks)}"
...
def main():
    ...
    parser.add_argument('--association-metric', default='cosine', 
                       choices=['cosine', 'euclidean', 'iou'], help='关联度量方式 (默认: cosine)')
    ...
    parser.add_argument('--color-scheme', default='id_based', 
    ...
    # 执行高级跟踪可视化
    result = track_and_visualize_advanced(
    ...
    if result:
        ...
        print("\n📋 快速参数调整示例:")
        print("  # 调整跟踪阈值:")
        print(f"  python {__file__} --conf 0.4 --track-conf 0.3")
        print("  # 修改关联度量:")
        print(f"  python {__file__} --association-metric iou")
        print("  # 调整轨迹显示:")
        print(f"  python {__file__} --trail-length 50 --fade-trail")
        print("  # 禁用卡尔曼滤波:")
        print(f"  python {__file__} --enable-kalman False")
        print("  # 调整跟踪敏感度:")
        print(f"  python {__file__} --min-hits 2 --max-age 40")
        print("  # 检测所有类别:")
        print(f"  python {__file__} --target-class all")
    else:
        print("\n❌ 处理失败!")
    ...
    try:
        ...
        while frame_count < process_frames:
            ...
            if frame_count % 50 == 0:
                ...
                print(f"⏳ 进度: {progress:.1f}% ({frame_count}/{process_frames}) - 检测: {len(detections)}, 活跃轨迹: {active_tracks}")
